...
def extract_features(file_path):
    try:
        ...
        # ③ ΔΔ（デルタ・デルタ）MFCCを計算（変化の加速度）
        delta2_mfcc = librosa.feature.delta(mfcc, order=2)
        
        # ④ 0番目（パワー成分）を省いて縦に結合
        # それぞれ13次元なので、13 + 13 + 13 = 合計39次元になります
        combined_mfcc = np.vstack([delta_mfcc[1:], delta2_mfcc[1:]])
        
        # ⑤ 時間方向の平均をとる
        mfcc_mean = np.mean(combined_mfcc.T, axis=0)
        return mfcc_mean
