...
def predict_batch(classifier, target_folder):
    ...
    print(f"--- 📂 一括判定開始 (2クラス専用): {target_folder} ---")
    ...
    for file_path in files:
        ...
        if feat is not None:
            ...
            # 2クラス学習の場合、出力される確率は2つだけ（[緊急の確率, それ以外の確率]）
            # ※学習データのラベルが [1, 2] なので、probs[0]がクラス1、probs[1]がクラス2になります
            probs = classifier.predict_proba(feat)[0]
            prob_normal = probs[0] # 緊急
            prob_other  = probs[1] # それ以外

            # ★ 拮抗判定（両方40%以上）
            if prob_normal >= 0.45 and prob_other >= 0.45:
                count_borderline += 1
            
            # 確率が高い方を判定結果にする
            if prob_normal > prob_other:
                result_str = "⚠️ 普通(低)"
                count_normal += 1
            else:
                result_str = "⚪ それ以外"
            ...
            # 詳細表示用の文字列
            detail_str = f"緊:{prob_normal*100:.0f}% 他:{prob_other*100:.0f}%"
    ...
    print("-" * 70)
    print("【 📊 判定結果まとめ 】")
    print(f"  ⚠️ 普通(低緊迫): {count_normal} 件")
    print(f"  ⚪ それ以外    : {count_other} 件")
    print(f"  -----------")
    print(f"  合計           : {len(files)} 件")
    print(f"  (※うち、「緊急」と「それ以外」が拮抗: {count_borderline} 件)")
