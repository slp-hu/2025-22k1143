...
def extract_features(file_path):
    try:
        ...
        mfcc_mean = np.mean(mfcc[1:].T, axis=0)
        return mfcc_mean
...
# モデル（classifier）の中に「estimators_」というリストがあり、そこに100本の木が入っています
# そのうちの「0番目（最初の1本）」を取り出して中身を見ます
if my_classifier:
    # 決定木を1つ取り出す
    one_tree = my_classifier.estimators_[12]
    
    # ルールをテキストとして書き出す
    # feature_namesにはMFCCの次元数(0~39)が入ります
    tree_rules = export_text(one_tree, feature_names=[f"MFCC_{i}" for i in range(39)])
